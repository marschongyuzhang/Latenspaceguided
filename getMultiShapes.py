...
class A:
    def readYmal(self, filepath, labeledImg=None):  #class
        if os.path.exists(filepath):
            ...
            self.classname = segmentation_double.img_path.split("/")[-1]
            #classname = classname.split(".")[0]
            self.classes.append(self.classname.split("_")[0])

            return zip(self.classes, labels)
        else:
...
def getMultiShapes_yaml(oriImgPath,
                   labelPath,
                   savePath='',
                   labelYamlPath='',
                   flag=False,
    ...
    labelShape = label_img.shape
    class_readymal = A()
    labels = class_readymal.readYmal(labelYamlPath, label_img)
    # print(list(labels))
    shapes = []
    obj = dict()
    obj['version'] = labelme_version
    ...
    # print(len(shapes))
    obj['shapes'] = [shapes[-1]] #changable[shapes[-1]] max(shape).list of/ low to high --> above
    # print(shapes)
